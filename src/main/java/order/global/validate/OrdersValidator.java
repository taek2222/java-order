...
public class OrdersValidator {
    ...
    public static void validateOrders(final List<Order> orders) {
        validateOrdersMinimum(orders);
        validateOnlyMenuTypeDrink(orders);
    }

    private static void validateOrdersMinimum(final List<Order> orders) {
        if (orders.size() < ORDERS_MINIMUM) {
            throw new IllegalArgumentException(INVALID_ORDERS_MINIMUM.get());
        }
    }

    private static void validateOnlyMenuTypeDrink(final List<Order> orders) {
